...
static int json_bool(cJSON* obj, const char* key, int* out_set, int* out_val) {
  cJSON* v = cJSON_GetObjectItemCaseSensitive(obj, key);
  if (!v) { *out_set = 0; *out_val = 0; return 0; }
  if (!cJSON_IsBool(v)) return 1;
  *out_set = 1;
  *out_val = cJSON_IsTrue(v) ? 1 : 0;
  return 0;
}

static int json_int64(cJSON* obj, const char* key, int* out_set, int64_t* out_val) {
  cJSON* v = cJSON_GetObjectItemCaseSensitive(obj, key);
  if (!v) { *out_set = 0; *out_val = 0; return 0; }
  if (!cJSON_IsNumber(v)) return 1;
  *out_set = 1;
  *out_val = (int64_t)v->valuedouble;
  return 0;
}
...
static int load_args_array(cJSON* arr, yai_law_arg_t** out_args, size_t* out_len) {
  *out_args = NULL; *out_len = 0;
  ...
  for (int i = 0; i < n; i++) {
    ...
    // default: support bool/int/string
    ...
    const char* artifacts_json_path)
{
  ...
  // load artifacts first (validate needs it)
  ...
  // choose top-level version/binary (prefer commands, fallback artifacts)
