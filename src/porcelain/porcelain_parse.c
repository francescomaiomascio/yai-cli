...
static const yai_law_command_t* find_by_group_name(const yai_law_registry_t* reg, const char* group, const char* name) {
...
int yai_porcelain_parse_argv(int argc, char** argv, yai_porcelain_request_t* req) {
  ...
  if (!cmd || !cmd->id) {
    ...
    if (lst.items && lst.len > 0) return set_err(req, "unknown command name for group (try: yai help <group>)");
