...
int yai_porcelain_parse_argv(int argc, char** argv, yai_porcelain_request_t* req) {
  ...
  if (strcmp(argv[cmdi], "lifecycle") == 0) {
    if (cmdi + 1 >= argc || !argv[cmdi + 1]) {
      return set_err(req, "missing lifecycle action", "Run: yai help lifecycle");
    }
    ...
    else return set_err(req, "unknown lifecycle action", "Run: yai help lifecycle");
