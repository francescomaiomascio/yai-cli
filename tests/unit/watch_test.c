...
static void test_target_resolution_defaults(void)
{
  ...
  assert(strcmp(t.resolved_target, "root ping") == 0);
  assert(strcmp(t.display_target, "root ping") == 0);
...
static void test_target_resolution_filters_watch_options(void)
{
  yai_watch_target_t t;
  char err[128];
  char *argv[] = {"root", "ping", "--interval-ms", "500", "--count", "3", "--no-clear"};
  assert(yai_watch_target_resolve(7, argv, &t, err, sizeof(err)) == 0);
  assert(strcmp(t.requested_target, "root ping") == 0);
  assert(strcmp(t.resolved_target, "root ping") == 0);
}
