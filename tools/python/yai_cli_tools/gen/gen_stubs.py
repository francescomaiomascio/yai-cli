...
ROOT = Path(__file__).resolve().parents[4]  # tools/python/yai_cli_tools/gen -> repo root
...
def _emit_cache_c(commands: dict[str, Any], artifacts: dict[str, Any], out_c: Path) -> None:
    ...
    # Pre-collect enum values arrays etc.
    c_lines: list[str] = []
    ...
    for idx, c in enumerate(cmds):
        ...
        sym = "cmd_" + "".join(ch if ch.isalnum() else "_" for ch in cid)
        ...
        for a_i, a in enumerate(args):
            ...
            # defaults: we support bool/int/string in a simple, explicit way
            default_s = None
            default_b_set = 0
            default_b = 0
            default_i_set = 0
...
def main() -> int:
    ...
    commands_path = law / "law" / "abi" / "registry" / "commands.v1.json"
    artifacts_path = law / "law" / "abi" / "registry" / "artifacts.v1.json"
    ...
    out_c = ROOT / "src" / "law" / "registry_cache.c"  # we overwrite the placeholder (by design)
...
if __name__ == "__main__":
    raise SystemExit(main())# tools/python/yai_cli_tools/gen/gen_stubs.py
from __future__ import annotations
